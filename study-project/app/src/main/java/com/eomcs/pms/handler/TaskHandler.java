...
public class TaskHandler {
  ...
  public void add() {
    ...
    if (project.getOwner().getNo() != AuthHandler.getLoginUser().getNo()) {
      System.out.println("이 프로젝트의 관리가 아닙니다.");
      return;
    }
    ...
    task.setOwner(MemberHandler.promptMember(
        "담당자?(취소: 빈 문자열) ", project.getMembers()));
  ...
  public void list() {
    ...
    Project project = projectHandler.promptProject();
    if (project == null) {
      System.out.println("작업 목록 보기를 취소합니다.");
      return;
    }
    printTasks(project);
  ...
  public void detail() {
    ...
    int taskNo = Prompt.inputInt("작업번호:?");
